...
While TTYloom is in beta, versions are numbered `v0.N-beta` (`v0.7-beta`, then `v0.8-beta`) and every GitHub release is marked as a pre-release.
...
3. Tag that commit and push it, replacing `v0.7-beta` below with the new version:

   ```bash
   git tag -a v0.7-beta -m 'TTYloom v0.7-beta'
...
5. Review its files and release notes, then publish the draft from GitHub or:
   ...
   ```bash
   gh release edit v0.7-beta --draft=false --prerelease --latest
   ```
...
```bash
bash scripts/release.sh v0.7-beta
cd dist/v0.7-beta
